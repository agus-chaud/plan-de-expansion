"""
Scraper de sucursales Jumbo en CABA.

Estrategia: API VTEX Master Data (REST) — mismo patrón que Disco (ambos son Cencosud).
Endpoint: /api/dataentities/NT/search con header REST-Range para paginación.
Coordenadas: campo geocoordinates (lat,lon) o lat/lon separados.
Filtro: grouping == 'Capital Federal'.
...
OUTPUT_FILE = OUTPUT_DIR / "jumbo.csv"

BASE_URL = "https://www.jumbo.com.ar/api/dataentities/NT/search"
FIELDS = "name,grouping,geocoordinates,state,city,street,number,neighborhood,address"
CABA_GROUPING = "Capital Federal"
...
def parsear_geocoordinates(raw: str) -> tuple[float | None, float | None]:
    """
    Parsea el campo geocoordinates de VTEX: '-34.6103015,-58.4482393'.
...
def fetch_all_stores() -> list[dict]:
    """Descarga todos los stores de Jumbo usando paginación con REST-Range."""
    all_stores = []
    batch_size = 100
    start = 0
...
def main():
    ...
    print(f"[INFO] Descargando stores Jumbo via API VTEX Master Data...")
    all_stores = fetch_all_stores()
    print(f"[INFO] Total stores Jumbo: {len(all_stores)}")
    ...
    for store in all_stores:
        ...
        # Filtrar solo Capital Federal
        if grouping != CABA_GROUPING:
            descartados += 1
            continue

        nombre = store.get("name", "Jumbo")
